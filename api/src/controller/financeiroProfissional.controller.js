...
async function dashboardGerencial(req, res) { try { const empresaId = Number(req.query.empresaId), hoje = new Date(), em30 = new Date(); em30.setDate(hoje.getDate() + 30); const [vendas, recebidas, despesas, contas, estoqueBaixo] = await Promise.all([prisma.venda.aggregate({ where: { empresaId, statusNfe: { not: "CANCELADA" } }, _sum: { total: true, lucro: true } }), prisma.caixa.aggregate({ where: { empresaId, tipoOperacao: "ENTRADA", status: "PAGO" }, _sum: { valorPago: true } }), prisma.caixa.aggregate({ where: { empresaId, tipoOperacao: "SAIDA", status: "PAGO" }, _sum: { valorPago: true } }), prisma.contaReceber.findMany({ where: { empresaId, status: { in: ["PENDENTE", "PARCIAL", "VENCIDO"] } }, select: { valorOriginal: true, valorRecebido: true, vencimento: true } }), prisma.produto.findMany({ where: { empresaId, ativo: true }, select: { estoque: true, estoqueMinimo: true } })]); const saldoPendente = contas.reduce((s,c)=>s+c.valorOriginal-c.valorRecebido,0), vencidas=contas.filter(c=>c.vencimento<hoje).reduce((s,c)=>s+c.valorOriginal-c.valorRecebido,0), previsao=contas.filter(c=>c.vencimento<=em30).reduce((s,c)=>s+c.valorOriginal-c.valorRecebido,0), produtosAbaixoMinimo=estoqueBaixo.filter(p=>Number(p.estoque)<=Number(p.estoqueMinimo)).length, saldoCaixa=n(recebidas._sum.valorPago)-n(despesas._sum.valorPago); res.json({ faturamento:n(vendas._sum.total), entradasRecebidas:n(recebidas._sum.valorPago), despesas:n(despesas._sum.valorPago), saldoCaixa, lucro:n(vendas._sum.lucro), contasVencidas:vencidas, contasAVencer:saldoPendente-vencidas, previsaoEntradas30Dias:previsao, previsaoSaldo30Dias:saldoCaixa+previsao, produtosAbaixoMinimo }); } catch(error) { console.error(error); res.status(500).json({ error:"Erro no dashboard gerencial." }); } }
